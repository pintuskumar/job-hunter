...
Candidate details live in the active profile rather than in Python source code.

1. Open `/profile` and select or import a preset.
2. Update the candidate name, bio, verified achievements, core technologies,
   outreach templates, and email greeting.
3. Use **Save & Activate** so collection, scoring, outreach, and email all use
   the same profile.

You can also edit a YAML preset in `profiles/` and import it from the Profile
page. Presets intentionally contain placeholders; replace them only with facts
...
short = (
    ...
    f"My background includes {stack_phrase}, and I would like to..."
)
```
