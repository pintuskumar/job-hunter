...
async function api(path, opts = {}) {
    ...
    init.headers = { ...jhAuthHeader(), ...init.headers };
    let resp = await fetch(`${API_BASE_URL}/api${path}`, init);
    if (resp.status === 401) {
        await jhHandleUnauthorized();
        init.headers = { ...init.headers, ...jhAuthHeader() };
        resp = await fetch(`${API_BASE_URL}/api${path}`, init);
    }
    if (!resp.ok) {
...
async function exportActiveProfile() {
    if (state.currentId == null) return;
    // Plain navigation — can't carry our Authorization header, so the browser
    // will show its own native Basic Auth prompt the first time.
    const url = `${API_BASE_URL}/api/profiles/${state.currentId}/export`;
